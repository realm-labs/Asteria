...
import io.github.realmlabs.asteria.event.stream.DurableEventDelivery
import io.github.realmlabs.asteria.event.stream.DurableEventEnvelope
import io.github.realmlabs.asteria.event.stream.DurableEventFailureMode
import io.github.realmlabs.asteria.event.stream.DurableEventHandler
import io.github.realmlabs.asteria.event.stream.DurableEventPublishResult
import io.github.realmlabs.asteria.event.stream.DurableEventStartPosition
import io.github.realmlabs.asteria.event.stream.DurableEventSubscribeOptions
import io.github.realmlabs.asteria.event.stream.DurableEventSubscription
import io.github.realmlabs.asteria.event.stream.DurableEventType
...
class NatsJetStreamEventBus(
    ...
    private fun DurableEventEnvelope.toHeaders(): Headers {
        ...
        headers.put(HEADER_EVENT_TYPE, type.value)
        headers.put(HEADER_EVENT_ID, eventId)
        headers.put(HEADER_OCCURRED_AT, occurredAt.toString())
        key?.let { headers.put(HEADER_EVENT_KEY, it) }
        correlationId?.let { headers.put(HEADER_CORRELATION_ID, it) }
        causationId?.let { headers.put(HEADER_CAUSATION_ID, it) }
        return headers
    }
    ...
    private fun Message.toEvent(stream: EventStreamName): DurableEventEnvelope {
        ...
        val type = headers.required(HEADER_EVENT_TYPE)
        val eventId = headers.required(HEADER_EVENT_ID)
        val occurredAt = headers.getFirst(HEADER_OCCURRED_AT)?.let(Instant::parse) ?: Instant.now()
        val eventHeaders = headers.toSingleValueMap() - ReservedHeaders
        return DurableEventEnvelope(
            stream = stream,
            type = DurableEventType(type),
            payload = data,
            key = headers.getFirst(HEADER_EVENT_KEY),
            headers = eventHeaders,
            eventId = eventId,
            occurredAt = occurredAt,
            correlationId = headers.getFirst(HEADER_CORRELATION_ID),
            causationId = headers.getFirst(HEADER_CAUSATION_ID),
        )
    }
}
...
private const val HEADER_EVENT_TYPE = "Asteria-Event-Type"
private const val HEADER_EVENT_ID = "Asteria-Event-Id"
private const val HEADER_EVENT_KEY = "Asteria-Event-Key"
private const val HEADER_OCCURRED_AT = "Asteria-Event-Occurred-At"
private const val HEADER_CORRELATION_ID = "Asteria-Event-Correlation-Id"
private const val HEADER_CAUSATION_ID = "Asteria-Event-Causation-Id"
