...
- `:config:config-core`: config table snapshot, reload, validation, change dispatch, and module contracts.
- `:config:config-annotations`: annotations used by config table accessor and change-handler code generation.
- `:config:config-gradle-plugin`: Gradle plugin that wires KSP and config code generator dependencies.
- `:config:config-ksp`: KSP processor that generates strongly typed config table refs, accessors, and change-handler lists.
...
class ActivityConfigChangeHandler : ConfigChangeHandler<PlayerActor> {
    ...
    override fun handleChange(actor: PlayerActor, event: ConfigChangedEvent) {
        syncActivities(actor, event.current)
    }

    override fun catchUp(actor: PlayerActor, snapshot: ConfigSnapshot) {
        syncActivities(actor, snapshot)
    }
}

val dispatcher = ConfigChangeDispatcher(GeneratedPlayerConfigChangeHandlers.ALL)

// Online hot reload event.
dispatcher.dispatchIfNew(actor, event, actor.configRevisionTracker)

// Actor starts, logs in, or becomes active after missing reload events.
dispatcher.catchUpIfNew(actor, configService.current(), actor.configRevisionTracker)
```

`ConfigChangeDispatcher` only decides which handlers run and when a revision is considered handled. Projects still
choose how config change events reach actors and where each actor stores its last handled revision.
