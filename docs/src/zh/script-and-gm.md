...
脚本模块用于运维脚本、GM 修复和批量诊断。GM 模块负责 feature 元数据、操作授权入口、审计上下文和 HTTP
starter。两者可以独立使用，也可以组合成完整后台能力。
...
业务 actor 需要实现脚本执行入口，通常是在 actor 内组合 `ActorScriptSupport`，并把 `ActorScriptSupport.receive()`
合并进允许执行脚本的 receive 状态。
...
`GmFeatureRegistry` 是不可变 feature catalog，会在启动时拒绝重复 feature id 和重复 action key，避免扩展模块静默覆盖彼此。
feature descriptor 只保存元数据：feature id/name/description、action、menu 和 route。可选模块通过 Java `ServiceLoader`
发布 feature；`gm-spring-boot-starter` 会把 ServiceLoader 发现的 feature 和 Spring bean 中的 `GmFeature` 合并后注册，并暴露
feature、action、menu、route 列表。具体业务 API 仍由各 starter 或业务 controller 提供。

`risk` 只是元数据标记，不会自动触发审批、MFA 或工单流。生产系统要在 `GmPrincipalResolver` 和
`GmAuthorizationPolicy` 中补上这些策略。
...
状态、超时、失败和 GM action 元数据，不绑定 PlayerActor、WorldActor、网关或进程退出语义。
...
GM 或运维入口可以从 `ServiceRegistry` 取出 `GmShutdownOperations`，提交 `GmShutdownRequest` 并查询 `status()`。完成业务
drain
...
`GmPrincipalResolver`，通常还要提供 `GmAuditSink` 和符合项目权限模型的 `GmAuthorizationPolicy`。默认 authorization policy
是拒绝所有操作。
