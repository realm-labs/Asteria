...
class LubanConfigModule private constructor(
    ...
    override suspend fun install(context: ModuleContext) {
        val loader = LubanJsonConfigLoader(
            tablesType = options.tablesType ?: error("Luban tables type must be configured"),
            dataDir = options.dataDir ?: error("Luban json data dir must be configured"),
...
data class LubanConfigModuleOptions(
    ...
    val revisionFactory: (LubanJsonLoadReport) -> ConfigRevision,
...
class LubanConfigModuleBuilder {
    ...
    private var revisionFactory: (LubanJsonLoadReport) -> ConfigRevision = { report ->
    ...
    fun revision(factory: (LubanJsonLoadReport) -> ConfigRevision) {
    ...
    internal fun build(): LubanConfigModuleOptions {
        ...
        return LubanConfigModuleOptions(
            ...
            charset = charset,
            fileResolver = { file ->
                val dir = dataDir ?: error("Luban json data dir must be configured")
                fileResolver?.invoke(dir, file) ?: dir.resolve("$file.json")
            },
